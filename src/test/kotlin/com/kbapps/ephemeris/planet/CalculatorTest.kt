...
class CalculatorTest {
    ...
    private fun testCalculator(
        ...
        val jdTimeStart = JulianDate.toJD(grDate.withHour(0).withMinute(0).withSecond(0).withNano(1))
        ...
        val planetPosition =
            model.calculatePlanetPosition(jdTime[0], JulianDate.tFromJD(jdTime[0], jdTime[1]), latitude, longitude)

        // calculate transit time
        val transitPosition = Calculator.calculateEventPosition(jdTime[1], model, planetDayPosition, 0.0)
        if (transitTime != null) {
            ...
            assertEquals(transitEl ?: 0.0, transitPosition!!.elevationInDeg, 0.01, "incorrect transit elevation")
        } else {
